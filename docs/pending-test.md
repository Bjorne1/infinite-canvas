...
- 云端渠道生图请求改为后端异步任务：前端先创建 `/api/v1/ai-tasks/...` 后台任务并轮询状态，任务完成后再读取原始上游响应；任务创建带客户端幂等 ID，避免网络波动下重复提交生图，后端任务完成结果会在进程内保留一段时间。
- 生图工作台支持全选/多选生成结果后批量分类，可批量添加/移除分类、创建新分类并应用到所选图片，或批量移至未分类；历史结果卡片默认只展示分类和生成时间，其余模型、接口、尺寸、质量、格式、审核、流式、超时和耗时参数折叠到“参数”里。
...
- 生图工作台支持连续提交多个并发生图任务，结果卡片展示任务时间、提示词、模型、API 模式、尺寸、质量、格式和耗时，失败任务展示上游报错并支持单任务重试。
